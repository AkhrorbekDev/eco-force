<template>
  ...
  <ModalWindow :isVisible="isModalVisible" @close="closeModal">
    <div class="popup">
      ...
      <p>
        {{ $t('После нажатия кнопки вы перейдете в диалог с нашим ботом') }} <a  :href="URL"
                                                                    target="_blank">
        <b>@EcoForceBot</b>
      </a>.
      </p>
      <p>
        {{ $t('В диалоге с ботом нажмите кнопку') }} <b>{{ $t('Start') }}</b>.
      </p>
      <a class="button button_blue py-12 w-100" :href="URL" target="_blank">
        ...
        <span>{{ $t('Войти через') }} <b>Telegram</b></span>
      </a>
    </div>
  </ModalWindow>
